...
In order to include in our analysis also more prolific MEPs on Twitter, the following sections focus on tweets posted in the eight-month period between May 2017 and December 2017. For the given period, our dataset includes data for all but the most hyper-active MEPs on the social network.
...
shareComparison <- bind_rows(SeatShare = MEPsDF %>% count(GroupShort) %>% mutate(ShareMep = n/sum(n)), 
          TweetShare = allMEPfull %>% filter(date>as.Date("2017-04-30"), date<as.Date("2018-01-01")) %>% count(GroupShort)  %>% mutate(ShareMep = n/sum(n)), .id = "Type") %>% 
  select(-n) %>% 
...
shareComparison %>%
  ...
  labs(title = "MEPs tweeting above their weight", subtitle = "Share of tweets posted since May 2017 VS\nshare of seats in the European parliament ")
```
...
shareComparison %>%
  ...
  scale_x_discrete("") +
  scale_y_continuous(name = "", limits = c(-1, 1), labels = scales::percent) +
  labs(title = "MEPs tweeting above their weight", subtitle = "Share of tweets posted since May 2017 (excluding retweets) VS share of seats in the EP")
```
...
shareComparison %>%
  ...
  labs(title = "MEPs tweeting above their weight", subtitle = "Share of retweets/favorite to posts by MEP since May 2017 VS share of seats in the EP")
```

The answer is quite clear: tweets by MEP belonging to largely eurosceptic EFDD and ENF attract by far more interactions than all others. To clarify the meaning of the above graph: if each MEP got exactly the same number of retweets and favorites in the last six months (no matter how much they tweeted), all groups would be at 0%. In other words, tweets by EFDD members attract 5 times more interactions than the average MEP. Tweets by the largest EP groups (EPP and S&D) attract far less interactions.
...
popularMEPsMedian %>%
  ...
  scale_x_discrete("") +
  scale_y_continuous(name = "", labels = scales::percent) +
  labs(title = "MEPs tweeting above their weight", subtitle = "Median number of retweets/favorite for each group VS median EP")
```
...
The following graph allows you to explore the number of followers of each MEP, and the number of interactions (retweets and favourite received) in the last eight months. 
