...
```
This is the first in a series of posts on how members of the European Parliament (MEPs) appear on Twitter. The first post will introduce the EP twittersphere, outline limitations to the analysis due to availability of tweets, and point at some broad trends. The following posts will focus on specific aspects.

The dataset of tweets at the base of this and the following posts consists of as much of the Twitter history of the current MEPs at Twitter makes available through its API; this amounts to `r scales::comma(nrow(allMEPfull))` tweets (yes, more than one million tweets).^[The code used to create the dataset is <a href="https://gist.github.com/giocomai/ea5fa8bd0732728abc05befedf8a2ac7">available on GitHub</a> An updated list of all MEPs with a Twitter account is available on <a href="https://github.com/eliflab/European-Parliament-Open-Data">GitHub, courtesy of Eliflab</a>.]
...
This post scratched the surface of the EP twittersphere just looking at how much MEPs tweeted at a given point in time. The next post will start to deal with the "who" question, and only later we'll finally loook at the actual contents of tweets.  
