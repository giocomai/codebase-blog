...
carto_lega_base <- cartogram_cont(sf::st_sf(scrutini_regione_geo %>% 
                                         filter(desc_lis == "LEGA SALVINI PREMIER")) %>%
                               select(voti, perc),
                             "voti",
                             itermax=7)

carto_5stelle_base <- cartogram_cont(sf::st_sf(scrutini_regione_geo %>% 
...
```{r combo_animated_solid, eval = FALSE}

combo_animated_solid <- morph %>%
  ggplot(mapping = aes(fill = perc)) +
  geom_sf() +
  coord_sf(datum = NULL) + 
  theme_void() +
  theme(legend.title=element_blank()) +
  transition_manual(frames = .frame) +
  labs(title = "Italy shaped as {if_else(.frame<1300, 'Lega', '5 stelle')}'s voters") 

combo_animated_solid
```
