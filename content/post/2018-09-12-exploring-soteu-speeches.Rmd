...
Same as above, with different color palette. 
...
#pal2 <- brewer.pal(8,"Dark2")
 speeches_by_speech %>% 
  ...
  with(wordcloud(word, n, max.words = 150, 
                 random.order = FALSE, 
                 colors = pal,  vfont=c("serif","plain")))
```
...
```{r}
keywords_circles_mostFrequent_noBarroso <- speeches_by_speech %>%
  filter(stringr::str_detect(string = id, pattern = "Barroso")==FALSE) %>% 
  filter(is.element(word, mostFrequentWords)) %>% 
  mutate(word = factor(word, levels = unique(word))) %>% # makes sure order is kept for graph 
  ggplot(mapping = aes(x = word, y = id, size = n, colour = n, label = n)) +
  geom_point(shape = 21, stroke = 3, size = 10) +
...
keywords_circles_mostFrequent_noBarroso
```
...
```{r wordcloud_all_soteu_byspeech_ss, fig.height=13, fig.width=14}
speeches %>%
  anti_join(stop_words) %>%
  group_by(id) %>% 
  count(word) %>%
  arrange(desc(id)) %>% 
  spread(id, n) %>% 
  select(word, `Barroso 2011`, `Barroso 2010`, `Juncker 2018`,`Juncker 2017`, `Juncker 2016`, `Juncker 2015`, `Barroso 2013`, `Barroso 2012`) %>% 
  remove_rownames() %>% 
...
keywords_circles <- speeches_by_speech %>% 
  filter(word == "solidarity" | word == "refugee" |word == "crisis" | word == "greece"| word == "climate" | word == "citizens"| word == "euro" | word == "jobs" | word == "africa" | word ==  "history") %>% 
  mutate(word = factor(word, levels = unique(word))) %>% # makes sure order is kept for graph 
  ggplot(mapping = aes(x = word, y = id, size = n, colour = n, label = n)) +
  geom_point(shape = 21, stroke = 3, size = 10) +
  geom_text(mapping = aes(size = 25), colour = "black") +
...
Looking at these wordclouds, it quickly appears that 'crisis' is the dominant negative words in each of the speeches, but the positive counterpoint is different every year. 
...
```{r}
library("gganimate")
#"solidarity" | word == "refugee" |word == "crisis" | word == "greece"| word == "climate" | word == "citizens"| word == "euro" | word == "jobs" 

speeches_by_speech %>% 
  mutate(id = str_replace(string = id, pattern = " ", replacement = "\n")) %>% 
 # filter(stringr::str_detect(string = id, pattern = "Barroso")==FALSE) %>% 
  filter(word == "refugee" |word == "crisis" | word == "growth") %>% 
  complete(id, word, fill = list(n = 0, total = 0)) %>% 
