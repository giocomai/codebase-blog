...
```{r carto_partiti_LEGA_5S_combo_animated}
...
```{r carto_partiti_PD_FI_animated}
...
# Less votes, less space

All of the above cartograms showed the distribution. 
...
complement_place <- function(scrutini, lista, tipo = "voti") {
  ...
  mutate(area = st_area(geometry)) %>% 
  mutate(voti_prop = voti*area, perc_prop = perc*area)
  rest_sf <- st_sf(tibble(desc_lis = lista, desc_reg = "out",
                          voti = voti_rest,
                          perc = perc_rest,
                          geometry = Moldova$geom))
  
  base_with_rest <- rbind(
    scrutini %>% filter(desc_lis==lista),
    rest_sf
  )
  
  carto_with_rest <- cartogram_cont(base_with_rest,
                                    weight = tipo,
                                    itermax = 25)
